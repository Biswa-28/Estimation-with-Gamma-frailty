...
marg_subdis_shared_cause <- function(t,j,mu_vec,tau_vec)
{
  ...
  return((cubature::cubintegrate(f_1,lower=0,upper=t,relTol=1e-9)$integral))
}
                                   ...
                                   mu_vec_2,tau_vec)
{
  ...
  {
    return(integrand_3(u,v,j_1,j_2,mu_vec_1,mu_vec_2,tau_vec))
  }
...
z <- function(v)
{
  I_1 <- sapply(v, g <- function(y)
  {
    g_1 <- function(u)
    {
      return(integrand_3(u,t_1=y,j_1=1,j_2=1,mu_vec_1=lambda_vec_1_initial,
      mu_vec_2=lambda_vec_2_initial,tau_vec=c(0.98,0.78,0.45)))
    }
    return(integrate(Vectorize(g_1),lower=0,upper=0.01)$value)
  })
  return(I_1)
}
integrate(z,0,0.01)$value
...
prob_cell_shared_cause <- function(x,j_1,j_2,mu_vec_1,mu_vec_2,tau_vec)
{
  ...
  {
    return(marg_subdis_shared_cause(x,j_1 - 1,mu_vec_1,tau_vec) - 
    sum(sapply(1:3,joint_sub_dis_shared_cause_1,t_3=x,t_4=x,j_3=j_1 - 1,
    mu_vec_3=mu_vec_1,mu_vec_4=mu_vec_2,tau_vec_1=tau_vec)))
  }else if((j_1 == 1)&&(j_2 > 1))
  {
    return(marg_subdis_shared_cause(x,j_2 - 1,mu_vec_2,tau_vec) - 
    sum(sapply(1:3,joint_sub_dis_shared_cause_1,t_3=x,t_4=x,j_4=j_2 - 1,
    mu_vec_3=mu_vec_1,mu_vec_4=mu_vec_2,tau_vec_1=tau_vec)))
  }else
  {
    return(joint_sub_dis_shared_cause_1(x,x,j_1 - 1,j_2 - 1,mu_vec_1,mu_vec_2,tau_vec))
  }
}
...
shared_cause_indiv_log_lik <- function(data,mu_vec_1,mu_vec_2,tau_vec)
{
  ...
  if((j_1 != 0) && (j_2 != 0))
  {
    h <- joint_sub_dis_shared_cause_1(x,x,j_1,j_2,
    ...
    h <- marg_subdis_shared_cause(x,j_1,mu_vec_1,tau_vec) - sum(sapply(1:3,joint_sub_dis_shared_cause_1,t_3=x,t_4=x,j_3=j_1,
         mu_vec_3 = mu_vec_1,mu_vec_4 = mu_vec_2,tau_vec_1=tau_vec))
    if(h > eps)
    {
      return(log(h))
    ...
    h <- marg_subdis_shared_cause(x,j_2,mu_vec_2,tau_vec) - 
      sum(sapply(1:3,joint_sub_dis_shared_cause_1,t_3=x,t_4=x,j_4=j_2,
                 mu_vec_3 = mu_vec_1,mu_vec_4 = mu_vec_2,tau_vec_1=tau_vec))
    if(h > eps)
    {
      return(log(h))
...
data_soln <- optimParallel::optimParallel(par=c(35.9582,0.0051,0.0057,39.2107,0.0053, 0.0041,12.9245,45.5892,18.2817),
...
clusterExport(cl,c("prob_cell_shared_cause","marg_subdis_shared_cause",
                   "integrand_2","integrand_3","joint_sub_dis_shared_cause_1","joint_surv_func",
                   "shared_cause_indiv_log_lik","parApply"))
data_soln_2 <- optimParallel::optimParallel(par=soln_2,
fn=neg_log_lik_shared_cause,M=3,D=hearing_data,lower=replicate(9,0.001),control=list(trace=6),hessian=TRUE,
parallel = list(cl=NULL,forward=FALSE,loginfo = TRUE),method = "L-BFGS-B")
hess <- data_soln_2$hessian
stopCluster(cl)
hess <- pracma::hessian(neg_log_lik_shared_cause,c(35.9582,0.00619684,0.00785706,39.2107,0.00568886,0.00840602,12.9245,45.5892,18.2817),D=hearing_data,M=3)
neg_log_lik_shared_cause(hearing_data,in_est_data,3)
neg_log_lik_shared_cause(hearing_data,data_soln,3)
eigen(hess)$values
